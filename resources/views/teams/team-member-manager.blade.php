        ...
        <x-action-section>
            <x-slot name="title">
                ...
                @if (Gate::check('addTeamMember', $team))
                    <x-button class="float-right" @click="show = true">Add User</x-button>
                @endif
            </x-slot>

            <x-slot name="description">
                {{ __('All the people that are part of this team.') }}

                @if (Gate::check('addTeamMember', $team))
                    <!-- Add Team Member -->
                    <div class="mt-10 sm:mt-0">
                        <x-form-section-modal submit="addTeamMember">
                            <x-slot name="modal">
                                <x-app-modal>
                                    <x-slot name="title">
                                        {{ __('Add Team Member') }}
                                            ...
                                            <div class="col-span-6 lg:col-span-4 mt-4">
                                                ...
                                                <div class="relative z-0 mt-1 border border-gray-200 rounded-lg cursor-pointer">
                                                    @foreach ($this->roles as $index => $role)
                                                        <button type="button" class="relative px-4 py-3 inline-flex w-full rounded-lg focus:z-10 focus:outline-none focus:border-black focus:ring-2 focus:ring-black {{ $index > 0 ? 'border-t border-gray-200 focus:border-none rounded-t-none' : '' }} {{ ! $loop->last ? 'rounded-b-none' : '' }}"
                                    ...
                                    <x-slot name="actions">
                                        ...
                                        <x-action-message class="text-r_green-200 mr-2 my-auto" on="saved">
                                            {{ __('Invitation Email Sent') }}
                                        </x-action-message>
                                    </x-slot>
                                </x-app-modal>
                            </x-slot>
                        </x-form-section-modal>
                    </div>
                @endif
            </x-slot>
                        ...
                        @foreach ($team->users->sortBy('name') as $user)
                            <div class="flex items-center justify-between">
                                <div class="flex items-center">
                                    ...
                                    <div class="ms-4">{{ $user->name }} <span class="ml-3 text-sm text-gray-500">{{ $user->email }}</span></div>
                                </div>
            ...
            @else
                <x-slot name="content">
                    <div class="text-gray-600">
                        {{ __('No other people are part of this team') }}
                    </div>
                </x-slot>
            @endif
...
{{--    </x-confirmation-modal>--}}
    ...
    <x-confirmation-modal wire:model.live="confirmingTeamMemberRemoval">
        ...
        <x-slot name="content">
            ...
            <h2 class="text-red-500">Please note that this will also delete the account of the removed user.</h2>
        </x-slot>
